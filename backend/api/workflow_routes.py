"""Workspace workflow definition and execution APIs."""
...
def _workspace_id(value) -> str:
    ...
    try: return validate_workspace_id(str(value or ""))
...
def register_workflow_routes(app) -> None:
    ...
    def workflows_collection():
        ...
        if not workspace_id: return jsonify({"ok": False, "error": "workspace_id is required"}), 400
        if request.method == "GET": return jsonify({"ok": True, "workflows": list_workflows(workspace_id)})
        try: workflow = save_workflow(workspace_id, data)
        except WorkflowError as exc: return jsonify({"ok": False, "error": str(exc)}), 400
    ...
    def workflow_runs(workflow_id):
        ...
        workspace_id = _workspace_id(request.args.get("workspace_id") or data.get("workspace_id"))
        if not workspace_id: return jsonify({"ok": False, "error": "workspace_id is required"}), 400
        if request.method == "GET": return jsonify({"ok": True, "runs": list_runs(workspace_id, workflow_id)})
        if data.get("enqueue"):
        ...
        try: run = execute_workflow(workspace_id, workflow_id, data.get("inputs") or {}, approvals=data.get("approvals") or {})
        except WorkflowError as exc: return jsonify({"ok": False, "error": str(exc)}), 400
    ...
    def workflow_run_detail(run_id):
        ...
        return (jsonify({"ok": True, "run": run}), 200) if run else (jsonify({"ok": False, "error": "workflow run not found"}), 404)
    ...
    def workflow_run_cancel(run_id):
        ...
        if not workspace_id: return jsonify({"ok": False, "error": "workspace_id is required"}), 400
        try: run = cancel_run(workspace_id, run_id)
        except WorkflowError as exc: return jsonify({"ok": False, "error": str(exc)}), 404
