...
def oidc_enabled() -> bool:
    return os.environ.get("LZCORE_OIDC_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}


def _public_url() -> str:
    value = os.environ.get("LZCORE_PUBLIC_URL", "").strip().rstrip("/")
    parsed = urlparse(value)
    if not value or parsed.scheme != "https" or not parsed.netloc:
        raise RuntimeError("LZCORE_PUBLIC_URL must be an absolute HTTPS URL for OIDC")
    return value


def _username_from_claims(claims: dict) -> str:
    claim_name = os.environ.get("LZCORE_OIDC_USERNAME_CLAIM", "preferred_username").strip()
...
def register_oidc_routes(app) -> None:
    ...
    issuer = os.environ.get("LZCORE_OIDC_ISSUER", "").strip().rstrip("/")
    client_id = os.environ.get("LZCORE_OIDC_CLIENT_ID", "").strip()
    from backend.core.auth import _secret_value
    client_secret = _secret_value("LZCORE_OIDC_CLIENT_SECRET")
    ...
    def oidc_callback():
        try:
            ...
            session["lzcore_auth_method"] = "oidc"
