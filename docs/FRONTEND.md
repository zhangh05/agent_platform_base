...
- 布局：Header 56px、Sidebar 176px、工作台进度轨 192px、普通页面最大宽度 1440px，均尊重 `--ui-scale: 0.8`。工作台的助手消息、工具调用卡片和输入框使用中间对话列可用宽度（消息为头像保留 44px），不设置固定阅读宽度上限；`--w-reading: 1120px` 仅约束空状态内容。收起进度轨时中央列继续扩展，窄屏保留时间线入口。
