...
批准后的批量配置不是隐式事务。设备明确返回某条命令失败时，驱动继续发送后续命令并在 `command_results` 中保留每一条结果。仅当传输已经断开、后续命令无法客观发送时，剩余命令才标为 `not_sent`，同时完整返回模型。写入结果不确定时不会自动重放；模型可依据完整记录决定 read-back 或后续操作。
