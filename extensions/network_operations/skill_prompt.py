...
def render_network_skill_prompt(context: dict[str, Any]) -> str:
    ...
    if write_enabled:
        contract = contract.replace("Read-only authorization remains enforced; configuration writes and destructive confirmation are not supported.",
                                    "Read actions remain strictly read-only. Configuration has separate opt-in Skill authority.")
        contract += """
...
- The Skill capability is the runtime authorization boundary, not blanket permission for unrelated changes. Act only on the user's requested configuration goal; first read current state, select only the necessary authorized devices, and execute explicit commands through the authorized connection. Do not substitute shell tools or alternate connections to bypass the Skill boundary.
