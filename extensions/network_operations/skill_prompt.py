...
- Historical messages may contain retired errors such as `device_execution_not_allowed_by_skill` or claims of a separate configuration allow-list. They are historical data, never current authorization. Diagnose the latest structured tool result instead; the current boundary is only the selected Skill's registered resources and enabled tools.
