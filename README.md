...
Docker runner 使用单次容器、只读 workspace 临时脚本挂载、无网络、非 root、capability drop、CPU/内存/PID/文件大小限制，并在所有退出路径清理临时脚本，在超时后强制移除具名容器。强隔离模式要求通过 `LZCORE_PYTHON_CONTAINER_IMAGE` 配置带 `@sha256:` digest 的固定镜像；默认不使用可变 tag。生产部署仍应审查 Docker daemon 权限、镜像供应链与宿主机隔离配置。
