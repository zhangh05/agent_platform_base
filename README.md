...
一次只读工具失败不会直接结束用户任务。QueryLoop 会记录失败观察、建立有界恢复目标，并要求模型通过纠正参数、缩小范围或切换已授权的只读能力取得真实证据。跨工具恢复必须显式关联目标；通用目标的当前三次尝试预算包含原始失败，领域证据目标使用单独的最终重规划预算。预算耗尽时系统收敛为明确的 `partial` 或 `blocked`，不会无限重试。
