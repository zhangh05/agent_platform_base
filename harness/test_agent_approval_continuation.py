...
def test_continuation_claim_is_durable_and_exactly_once(monkeypatch, tmp_path):
...
def test_ordinary_approval_returns_pending_without_waiting(monkeypatch, tmp_path):
    ...
    class Store:
        def create(self, **kwargs):
            ...
            return SimpleNamespace(approval_id=f"apr-{len(created)}")
    ...
    assert result["approval_ids"] == ["apr-1"]
