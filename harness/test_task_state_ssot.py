...
def test_reconcile_legacy_parallel_read_timeout_states_closes_only_safe_records(monkeypatch, tmp_path):
    monkeypatch.setenv("LZCORE_WORKSPACE_ROOT", str(tmp_path))
    import agent.runtime.task_state as task_state

    workspace_id = "ws-legacy-read-timeout"
    safe_session_id = "session-legacy-read-timeout"
    unsafe_session_id = "session-legacy-write-timeout"
    for session_id, side_effecting in ((safe_session_id, False), (unsafe_session_id, True)):
        contract = task_state.begin_task_state(
            workspace_id=workspace_id,
            session_id=session_id,
        ...
        assert contract is not None
        state = task_state.load_task_state(workspace_id, session_id)
        task = dict(state["task"])
        task.update({
            "status": "waiting_user",
            "next_action": "resolve_blocking_unknown",
            "pending_mutation_keys": [],
            "nodes": [{
                "node_id": "tool:legacy-timeout",
                "status": "failed",
                "side_effecting": side_effecting,
                "result_ref": "parallel tool layer exceeded its execution budget; outcomes may be uncertain",
            }],
            "unknowns": [{"kind": "blocking_unknown", "reason": "runtime_reported_blocking_unknown"}],
        })
        state["task"] = task
        task_state.atomic_write_json(task_state._state_path(workspace_id, session_id), state)

    outcome = task_state.reconcile_legacy_parallel_read_timeout_states(workspace_id)
    assert outcome == {"reconciled": 1, "skipped": 0}
    safe = task_state.load_task_state(workspace_id, safe_session_id)["task"]
    assert safe["status"] == "failed"
    assert safe["next_action"] == "retry_with_bounded_plan"
    assert safe["legacy_recovery"] == "parallel_read_timeout_safe_to_retry"
    assert safe["unknowns"] == []
    assert task_state.list_task_events(workspace_id, safe_session_id)[-1]["event_type"] == "task_reconciled"
    unsafe = task_state.load_task_state(workspace_id, unsafe_session_id)["task"]
    assert unsafe["status"] == "waiting_user"
    assert task_state.reconcile_legacy_parallel_read_timeout_states(workspace_id) == {
        "reconciled": 0,
        "skipped": 0,
    }
