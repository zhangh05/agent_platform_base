    ...
    <div className="page-body">
      ...
      <div className="workflow-layout">
        ...
        <main className="workflow-editor">
          ...
          {selected ? <section className="workflow-run-box"><div><h3>试运行</h3><p>输入本次流程需要的数据，结果会按依赖关系执行并可追踪；互不依赖的读取步骤将自动并行。</p></div><textarea className="input" rows={4} value={inputText} onChange={(event) => setInputText(event.target.value)} /><button className="btn primary" onClick={() => void run()} disabled={busy}>运行流程</button>{lastRun ? <div className={`workflow-run-result ${lastRun.status}`}><b>{lastRun.status === "succeeded" ? "运行成功" : "运行未完成"}</b>{lastRun.nodes.map((node) => <span key={node.node_id}>{node.node_id} · {node.status}{node.orchestration?.layer ? ` · 第 ${node.orchestration.layer} 组` : ""}{node.orchestration?.parallel ? " · 并行" : ""}</span>)}</div> : null}</section> : null}
        </main>
      </div>
    </div>
  </div>;
