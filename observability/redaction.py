...
def redact_trace_event(event: dict) -> dict:
    ...
    # Strip full source/output payloads from metadata. Keep old source_config
    # key covered so traces from older callers are still safe.
    for key in ("source_config", "raw_config", "raw_source", "source_raw", "raw_output", "raw_payload", "config"):
        if key in result.get("metadata", {}):
            result["metadata"][key] = "[REDACTED_FULL_SOURCE]"
            has_secret = True
